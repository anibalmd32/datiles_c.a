...
export class Model<T> implements IModel<T> {
    ...
    async select(options: ModelSelectOptions<T>, params: Array<any>) {
        ...
        const query = new QueryBuilder<T>(this.table).fromQuery(this.table).selectQuery(select);

        if (filters) query.whereQuery(filters);
        if (order) query.orderQuey(order);
        if (pagination) query.limitQuery(pagination.size).offsetQuery(pagination.skip);
    ...
    async count(options: ModelCountOptions<T>, params: Array<any>) {
        ...
        const result = await db.select<Array<{ total: number }>>(query.build(), params);
    ...
    async update(options: ModelUpdateOptions<T>, params: Array<any>) {
        const db = await Database.load('sqlite:datiles.db')
        const query = new QueryBuilder<T>(this.table).updateQuery(options.data, options.id)
